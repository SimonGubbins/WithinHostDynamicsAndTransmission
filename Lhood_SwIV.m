function [logL, prior]=Lhood_SwIV(par,tStart,tStop,D,tObs,VI,mFlag)
%
% [logL, prior]=Lhood_SwIV(par,tStart,tStop,D,tObs,VI,mFlag)
%
% Matlab function for computing the log likelihood for a model linking
% within-host dynamics and transmission of SwIV in pigs based on data from
% a series of one-to-one challenge experiments
%
% Inputs:
...
if mFlag==1 || mFlag==2
    g=exp(par(end-1));
elseif mFlag==3
    g=exp(par(4*nAnim+9:5*nAnim+8));
    p_g=[par(end-2); par(end-1)];
elseif mFlag==4
    g=par(4*nAnim+9:5*nAnim+8);
    p_g=[par(end-2); par(end-1)];
end
...
% Non-informative for infectiousness scaling
if mFlag==1
...
% For the models in which shedding is proportional to log titre, constrain
% the virus curves so the animals are not immediately infectious
if mFlag==2 || mFlag==4
    t01=tp-(1./lg).*log(2*Vp);
    t01(t01>=0)=1;
    t01(t01<0)=0;
    prior=prior+sum(log(t01));
end
%==========================================================================
...
% Compute the expected viral titre at each observation
    muVI=2.*Vp(a)./(exp(-lg(a).*(tObs(x)-tp(a)))+exp(ld(a).*(tObs(x)-tp(a))));
    muVI=log10(muVI);
