function ParEst_SwIV(mFlag,seeds,nsamp,nburnin,nthin)
%
% ParEst(mFlag,seeds,nsamp,nburnin,nthin)
%
% Matlab function to find implement a Bayesian MCMC scheme to estimate
% parameters linking within-host dynamics and transmission of SwIV in pigs
% using a simple phenomenological model.
%
% Inputs:
...
%         chain (note: number of chains is equal to the length of seeds)
...
varload=load('SwIV_TransmissionExperimentData');
...
varload=load('SwIV_VirusIsolationData');
...
% Compute the log-likelihood
        [CurrL, prior]=Lhood_SwIV(par,tStart,tStop,D,tObs,VI,mFlag);
...
% Compute the log likelihood and prior for the new parameter set
        [NewL, prior_new]=Lhood_SwIV(par_new,tStart,tStop,D,tObs,VI,mFlag);
...
Dhat=-2*Lhood_SwIV(mean(PS,1)',tStart,tStop,D,tObs,VI,mFlag);
...
% Save the outputs
save(['SwIVModel' num2str(mFlag) '_MCMCSamples'],...
